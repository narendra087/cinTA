...
public class CekHC {
    ...
    boolean Cons1(String type, int angka1, int angka2, int TS, int Rom, Kelas kls, ArrayList<Integer> distriKls) {
        if (type.contains("WorkDay")) {
            ...
            return MaxdayLoad(TS, angka1, kls, distriKls);
...
//            if (kls.getClassID() != listDT.get(i)) {
    ...
    boolean MaxdayLoad(int TS, int S, Kelas kls, ArrayList<Integer> listMDL) {
    ...
    private int DayLoad(int S, int lengthA, ArrayList<Integer> daysA,
            ArrayList<Integer> weeksA, ArrayList<Integer> listMDL) {                                         //DayLoad(d,w) = ∑i {Ci.length | (Ci.days and 2^d) ≠ 0 ∧ (Ci.weeks and 2^w) ≠ 0)}
        int load = 0;
        for (int i = 0; i < listMDL.size(); i++) {
            Kelas klsB = SearchKls(listMDL.get(i));
            int iTS = klsB.getTs();
            if (iTS != -1) {
                ArrayList<Integer> weeksB = klsB.getAvailableTS().get(iTS).getWeeks();
                ArrayList<Integer> daysB = klsB.getAvailableTS().get(iTS).getDays();
